...
class ValueHandler(core.Handler):
    ...
    def handle_line(self, line, current_line):
        ...
        if array_ref is not None:
            ...
            if line_type.startswith("array") and line[3].isdigit() and (id not in self.array_types or int(line[3]) != 0):
                self.array_types[id] = int(line[3])
            elif line_type.startswith("assign_") and line[9].isdigit() and (id not in self.array_types or int(line[9]) != 0):
